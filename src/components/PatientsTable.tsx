...
export default function PatientsTable({ patients }: { patients: PatientSample[] }) {
                ...
                <div className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                        <div className="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                            <table className="min-w-full w-full divide-y divide-gray-300"> {/* Ensure full width */}
                                ...
                                {patients.map((patient) => (
                                    <tr onClick={() => router.push(`/patients/${patient.uuid}`) } key={patient.email} className={'hover:bg-gray-100 cursor-pointer'}>
                                        ...
                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{patient.bloodGroup}</td>
                                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{patient.address}</td>
                                        <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                            <a href="#" className="text-indigo-600 hover:text-indigo-900">
                                                Edit<span className="sr-only">, {patient.fullName}</span>
                                            </a>
                                        </td>
                                    </tr>
                                ))}
